...
class UserPolicy
{
    ...
    {
        return $user->can('update user') && !$model->hasRole('SUPER_ADMIN') && $model->isNot($user);
    }

    public function delete(Authorizable $user, User $model)
    {
        return $user->can('delete user') && !$model->hasRole('SUPER_ADMIN') && $model->isNot($user);
    }

    public function changeStatus(Authorizable $user, User $model)
    {
        return $user->can('changeStatus user') && !$model->hasRole('SUPER_ADMIN') && $model->isNot($user);
    }

    public function resetPassword(Authorizable $user, User $model)
    {
        return $user->can('resetPassword user') && !$model->hasRole('SUPER_ADMIN') && $model->isNot($user);
    }
}
